...
import WeaponItems from "./WeaponItems.cdc"
import GnftToken from "./GnftToken.cdc"
import FlowToken from "./FlowToken.cdc"
import FUSD from "./FUSD.cdc"
import WeaponItems7 from "./WeaponItems7.cdc"
...
import WeaponItems7 from 0xbf69452890a74d8f
*/
pub contract WeaponItems7ForTheCombatGameRentalRegular {
  // Events
  ...
  pub resource PromiseCollection: PromiseCollectionPublic {
    ...
    access(contract) fun ownedBy(tokenId: UInt64, owner: Address): Bool {
      let collection = getAccount(owner)
        ...
        .borrow<&WeaponItems7.Collection{WeaponItems7.WeaponItems7CollectionPublic}>()
    ...
    access(contract) fun setView(tokenId: UInt64, owner: Address) {
      ...
      let view =  WeaponItems7ForTheCombatGameRentalRegular.NFTView(
        id: nftView.id,
        uuid: nftView.uuid,
  ...
  pub fun listForRent(acct: AuthAccount, tokenId: UInt64, endTime: UFix64, rentPerDay: UFix64, rentTokenType: String, guaranteePayment: @GnftToken.Vault) {
    ...
    if let collection = acct
      .getCapability<&WeaponItems7.Collection{NonFungibleToken.CollectionPublic, WeaponItems7.WeaponItems7CollectionPublic}>(WeaponItems7.CollectionPublicPath)
      .borrow() {
