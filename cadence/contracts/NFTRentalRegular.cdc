...
// import WeaponItems from 0xbf69452890a74d8f
...
pub contract NFTRentalRegular {
  ...
  pub resource PromiseCollection: PromiseCollectionPublic {
    ...
    access(contract) fun ownedBy(tokenId: UInt64, owner: Address): Bool {
      ...
      return collection.borrowWeaponItem(id: tokenId)! != nil
    }
    ...
    access(contract) fun setView(tokenId: UInt64, owner: Address) {
      ...
      let view =  NFTRentalRegular.NFTView(
        id: nftView.id,
        uuid: nftView.uuid,
  ...
  pub fun listForRent(acct: AuthAccount, tokenId: UInt64, endTime: UFix64, rentPerDay: UFix64, rentTokenType: String, guaranteePayment: @GnftToken.Vault) {
    ...
    if let collection = acct
      .getCapability<&WeaponItems.Collection{NonFungibleToken.CollectionPublic, WeaponItems.WeaponItemsCollectionPublic}>(WeaponItems.CollectionPublicPath)
      .borrow() {
